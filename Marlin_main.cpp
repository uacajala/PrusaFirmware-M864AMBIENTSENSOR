...
//!@n M864 - Wait for AMBIENT thermistor to reach target temperature.
...
#ifdef AMBIENT_THERMISTOR
	case 864: // M864 - Wait for AMBIENT thermistor to reach target temperature.
	{
		int set_target_ambient = 0;

		if (code_seen('S')) {
			set_target_ambient = code_value();
		}
		else {
			break;
		}
		...
		SERIAL_PROTOCOLPGM("Wait for AMBIENT target temperature:");
		SERIAL_PROTOCOL(set_target_ambient);
		SERIAL_PROTOCOLLN("");

		codenum = _millis();
		cancel_heatup = false;

		bool is_ambient_cooling = false;
		if ((degTargetBed() == 0) && (degTargetHotend(0) == 0)) {
		    is_ambient_cooling = true;
		}

		while ( ((!is_ambient_cooling) && (!cancel_heatup) && (current_temperature_ambient < set_target_ambient)) || (is_ambient_cooling && (current_temperature_ambient > set_target_ambient)) ) {
			if ((_millis() - codenum) > 1000) //Print Temp Reading every 1 second while waiting.
			{
				SERIAL_PROTOCOLPGM("A:");
				SERIAL_PROTOCOL_F(current_temperature_ambient, 1);
